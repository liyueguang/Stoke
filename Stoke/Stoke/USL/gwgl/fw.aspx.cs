    ...
    public partial class fw : System.Web.UI.Page
    {
        ...
        private void Field_Bind(DataTable dt)
        {
            ...
            for (int i = 0; i < _field_bind.Split(';').Length; i++)
            {
                ...
                if (StyleControl.GetType().ToString() == "System.Web.UI.WebControls.TextBox")
                {
                    ...
                    if (name == "p" || name == "a" || name == "b" || name == "c" || name == "w" || name == "s" || name == "m")
            ...
            switch (step_id)
            {
                ...
                case 4:
                    if (n.Text != String.Empty)
                        n.Text += ",";
                    n.Text += _zgxm;
                    this.tsyj.Text = "无异议"; //2010.04.12 会签默认意见
                    break;
                case 5:
                    t.Text = _zgxm;
                    break;
                case 6:
                    k.Text = _zgxm;
                    o.Text = DateTime.Now.ToLongDateString();
                    break;
                case 7:
                    this.TD1.Visible = true;
                    this.TD2.Visible = true;
                    ...
                    break;
                case 9:
                    h.Text = _zgxm; //承办人可以删除和添加附件 2009/06/01 wy
                    this.TD1.Visible = true;
                    this.TD2.Visible = true;
                    this.TD3.Visible = true;
                    this.TD4.Visible = true;
        ...
        protected void SaveYj()
        {
            if (tsyj.Text != string.Empty)
            {
                ...
                string _zgxm = GetName(_zgbh); //2010.04.12 删除原有意见
                //				string sql="delete from Dsoc_Flow_Gwyj where Flow_id='"+flow_id+"'and Doc_id='"+doc_id+"'and Step_id='"+step_id+"'and Yjr='"+_zgxm+"'";
                //				SqlCommand cmd = new SqlCommand(sql,conn);  
                //				cmd.ExecuteNonQuery();
                ...
                case 1:
                    {
                        ...
                        else if (s.Text == string.Empty)
                            Response.Write("<script>alert('请填写主题词！')</script>");
                        else if (a.Text == string.Empty)
                            Response.Write("<script>alert('请填写主送！')</script>");
                        else
                        {
                            SaveData();
                            ScfjToMSSQL();
                            string _URL = "../Workflow/Work_Next_Step.aspx?flow_id=2&step_id=" + step_id.ToString() + "&doc_id=" + doc_id.ToString() + "&zgbh=" + _zgbh.ToString();
        ...
        private void btn_upload_Click(object sender, System.EventArgs e)
        {
            ...
            if (Session["attachList"] == null && this.FileList.Items.Count != 0)
            {
