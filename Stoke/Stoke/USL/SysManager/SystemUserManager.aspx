...
<head runat="server">
    <title>添加用户</title>
    <script>
        function addVerify() {            var loginName = document.getElementById("<%=loginNameTbx.ClientID%>").value;            if(loginName == '') {		        alert('用户编号不能为空！');		        return false;	        }	        var pattern1 = /^[0-9]{4}$/;  
            if (!pattern1.test(loginName)) {  
                alert("用户编号是长度为4位的数字"); 
                return false;  
            }	        	        	        var name = document.getElementById("<%=nameTbx.ClientID%>").value;            if(name == '') {		        alert('用户真实姓名不能为空！');		        return false;	        }                        	        var pwd = document.getElementById("<%=pwdTbx.ClientID%>").value;	        var pwdVerify = document.getElementById("<%=verifyPwdTbx.ClientID%>").value;	        if(pwd == '') {		        alert('密码不能为空！');		        return false;	        }	        if(pwdVerify == '') {		        alert('确认密码不能为空！');		        return false;	        }	        var patrn = /^[0-9a-zA-Z_]\w{5,24}$/;	        if(!patrn.exec(pwd)) {		        alert('密码应该由6-25位数字或字母构成！');		        return false;	        }	        if(pwd != pwdVerify) {		        alert('两次输入密码不一致！');		        return false;	        }	        	        var email = document.getElementById("<%=EmailTbx.ClientID%>").value;	        var pattern = /^([\.a-zA-Z0-9_-])+@([a-zA-Z0-9_-])+(\.[a-zA-Z0-9_-])+/;  
